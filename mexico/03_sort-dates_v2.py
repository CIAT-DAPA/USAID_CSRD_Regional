...
if len(sys.argv) < 2:
	os.system('cls')
	print "\n Too few args"
	print "   - ie: python 03_sort-dates_v2.py S:\\observed\\weather_station\\mex-smn\\new_2018\\daily-raw"
	sys.exit(1)
...
varlist = "prec", "tmax", "tmin", "evap", "rhum"
# var = "tmin"
...
for var in varlist:
	...
	for st in stlist:
		...
		if not os.path.exists(yFile):
			...
			yFile.write(os.path.basename(st) + "\t" + var + "\t" + lines[1].split("\t")[0] + "\t" + lines[-1].split("\t")[0] + "\n")
		else:
			yFile = open(yFile,'a')
			yFile.write(os.path.basename(st) + "\t" + var + "\t" + lines[1].split("\t")[0] + "\t" + lines[-1].split("\t")[0] + "\n")
